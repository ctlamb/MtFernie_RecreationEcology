...
## check times for MST vs MDT. Use noon timed? Or daylight in images correlated with sunset?
## Heikos:MDT
## Fernie Rec: MDT
...
lm_results4 <- mod.dat.spat %>%
  ...
  group_modify(~ {
    ...
    if(species %in%c("elk", "white-tailed deer")){
    presence <- glmmTMB(presence ~ location_comments + MAP_scale + (1 | CameraModel) + (1 | month) + (1 | year) + (1 | location),
      offset = log(n_days_effort),
      data = .x %>% mutate(presence = ifelse(counts == 0, 0, 1)),
      family = binomial(link = "logit")
    )

    abundance <- glmmTMB(counts ~ location_comments + MAP_scale + (1 | CameraModel) + (1 | month) + (1 | year) + (1 | location),
      offset = log(n_days_effort),
      data = .x %>% filter(counts > 0),
      family = truncated_poisson()
    )
    }else if(species %in%c("mule deer")){
      presence <- glmmTMB(presence ~ location_comments + evi_scale +  (1 | CameraModel) + (1 | month) + (1 | year) + (1 | location),
                          offset = log(n_days_effort),
                          data = .x %>% mutate(presence = ifelse(counts == 0, 0, 1)),
                          family = binomial(link = "logit")
      )
      
      abundance <- glmmTMB(counts ~ location_comments + evi_scale +  (1 | CameraModel) + (1 | month) + (1 | year) + (1 | location),
                           offset = log(n_days_effort),
                           data = .x %>% filter(counts > 0),
                           family = truncated_poisson()
      )
    }else{
      presence <- glmmTMB(presence ~ location_comments +  (1 | CameraModel) + (1 | month) + (1 | year) + (1 | location),
                          offset = log(n_days_effort),
                          data = .x %>% mutate(presence = ifelse(counts == 0, 0, 1)),
                          family = binomial(link = "logit")
      )
      
      abundance <- glmmTMB(counts ~ location_comments +  (1 | CameraModel) + (1 | month) + (1 | year) + (1 | location),
                           offset = log(n_days_effort),
                           data = .x %>% filter(counts > 0),
                           family = truncated_poisson()
      )
    ...
    newdat <- data.frame(
      ...
      month = NA, # Random effects not included in marginal predictions
      year = NA,
      location = NA,
      evi_scale=0,
      MAP_scale=0,
    ...
    pred_abund <- predict(abundance, newdat, type = "response", se.fit = TRUE, re.form = NA)
    ...
    zi_link <- predict(presence, newdat, type = "response", se.fit = TRUE, re.form = NA)
...
get_two_contrasts <- function(dat, spname) {
  ...
  if(spname %in%c("elk", "white-tailed deer")){
    presence <- glmmTMB(
      presence ~ location_comments + MAP_scale + (1 | CameraModel) + (1 | month) + (1 | year) + (1 | location),
      offset = log(n_days_effort),
      family = binomial(link = "logit"),
      data = dat
    )
    
    # Fit abundance model (non-zero counts only)
    abundance <- glmmTMB(
      counts ~ location_comments + MAP_scale +  (1 | CameraModel) + (1 | month) + (1 | year) + (1 | location),
      offset = log(n_days_effort),
      family = truncated_poisson(),
    ...
    presence <- glmmTMB(
      presence ~ location_comments + evi_scale + (1 | CameraModel) + (1 | month) + (1 | year) + (1 | location),
      offset = log(n_days_effort),
      family = binomial(link = "logit"),
      data = dat
    )
    ...
    # Fit abundance model (non-zero counts only)
    abundance <- glmmTMB(
      counts ~ location_comments + evi_scale +  (1 | CameraModel) + (1 | month) + (1 | year) + (1 | location),
      offset = log(n_days_effort),
      family = truncated_poisson(),
      data = dat %>% filter(counts > 0)
    )
    ...
    presence <- glmmTMB(
      presence ~ location_comments + (1 | CameraModel) + (1 | month) + (1 | year) + (1 | location),
      offset = log(n_days_effort),
      family = binomial(link = "logit"),
      data = dat
    )
    ...
    # Fit abundance model (non-zero counts only)
    abundance <- glmmTMB(
      counts ~ location_comments + (1 | CameraModel) + (1 | month) + (1 | year) + (1 | location),
      offset = log(n_days_effort),
      family = truncated_poisson(),
      data = dat %>% filter(counts > 0)
    )
...
#     is.na(p.value) & species_common_name == "bike" & contrast != "Landscape_vs_Wildlife" ~ "**",
#     TRUE ~ label
#   ))
...
plot_labels <- pairwise_df_raw %>%
  ...
  mutate(
    ...
    comp_label = case_when(
      presence != "ns" & abundance != "ns" ~ paste0("p", presence, ", ", "c", abundance),
      presence != "ns" & abundance == "ns" ~ paste0("p", presence),
      presence == "ns" & abundance != "ns" ~ paste0("c", abundance),
      presence == "ns" & abundance == "ns" ~ ""
    )
  )
...
##do again but over top of raw data

# ─── 4. MAX ERROR‐BAR HEIGHT PER SPECIES ──────────────────────────────────────────
max_err <- plot.dat %>% filter(species_common_name %in% wild_species.all) %>%
  group_by(species_common_name, location_comments) %>%
  summarise(mean=mean(hitrate),
            se=sd(hitrate)/sqrt(n()))%>%
  group_by(species_common_name) %>%
  summarize(y_max = max(mean+se, na.rm = TRUE), .groups = "drop")
...
# pick your palette
pal <- MetBrewer::met.brewer("Hokusai1", length(my_sp_order))

loop.dat %>%
  filter(!term %in% c("sd__(Intercept)", "(Intercept)")) %>%
  mutate(
    type = fct_relevel(type, "presence", "abundance"),
    term = case_when(
...
wild_species <- c("moose", "black bear", "grizzly bear")
